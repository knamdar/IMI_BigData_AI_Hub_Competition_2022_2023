...
def experiment(threshold):
    ...
    for seed in range(N):
        ...
        clf, auc_xgb = supervised_model_training(X, y, clf, seed, threshold)
        ...
        threshold_xgb_perf.append(auc_xgb)
        threshold_catboost_perf.append(auc_catboost)
    return threshold_xgb_perf, threshold_catboost_perf
...
# #########################Balanced Dataset####################################
    ...
    xgb_aucs = []
    catboost_aucs = []
    for threshold in np.linspace(0.1, 1, 19):
        print("working on thershold", threshold)
        xgb_auc, catboost_auc = experiment(threshold)
        xgb_aucs.append(xgb_auc)
        catboost_aucs.append(catboost_auc)
    pickle.dump(xgb_aucs, open("../results/xgb_aucs.p", "wb"))
    pickle.dump(catboost_aucs, open("../results/catboost_aucs.p", "wb"))
