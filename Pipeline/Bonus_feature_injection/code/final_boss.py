...
def balance_graph(lbls, node_ids):
    ...
    high_risk_nids = nids[labels==2]
    mid_risk_nids = nids[labels==1]
    low_risk_nids = nids[labels==0]
    mid_risk_to_keep = np.random.choice(mid_risk_nids, size=high_risk_nids.shape[0], replace=False)
    low_risk_to_keep = np.random.choice(low_risk_nids, size=high_risk_nids.shape[0], replace=False)
    balanced_nodeids = np.concatenate((high_risk_nids, mid_risk_to_keep,low_risk_to_keep), axis=0)
    balanced_lbls = np.concatenate((labels[labels==2], np.ones(high_risk_nids.shape[0]), np.zeros(high_risk_nids.shape[0])), axis=0)
...
if __name__ == "__main__":
    ...
    baseline_auc = roc_auc_score(y_test, predictions, multi_class="ovr")
    ...
    clf.fit(X_dev, y_dev)
    predictions = clf.predict_proba(X_test)
    new_auc = roc_auc_score(y_test, predictions, multi_class="ovr")
